...
def step_given_injector_of_static_prefix(context):
    context.injector = prefix_injector(prefix=context.prefix,
...
def step_given_injector_of_autogenerated_prefix(context):
    context.injector = autoprefix_injector()
...
def step_given_functions(context):
    ...
    def bar():
        with prefix_injector("bar") as inj:
            result = [inj.mark("bar string"), ]
            result.extend(foo())
            return result

    def baz():
        with prefix_injector("baz") as inj:
            result = [inj.mark("baz string"), ]
            result.extend(foo())
            return result

    def qux():
        with prefix_injector("qux") as inj:
